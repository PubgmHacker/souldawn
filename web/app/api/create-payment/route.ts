 ...
 * сумма считается на сервере. Промокод применяется серверно. Это исключает
 * подмену цены/суммы на клиенте.
 */
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { items, contact, payment_method, promo_code, delivery } = body as {
      items: IncomingItem[];
      contact: Contact & {
        city?: string;
        region?: string;
    ...
    for (const it of items) {
      ...
      subtotalKopecks += product.priceKopecks * qty;
      serverItems.push({
        id: product.id,
        name: product.name,
        size: it.size || "",
    ...
    // Доставка обязательна: СДЭК или курьер. Самовывоза нет.
    ...
    // Сохраняем заказ в БД (копейки, серверные цены). Статус всегда pending.
    try {
